...
<?php
	...
	get_template_part( 'template-parts/page-title/page-title-category', get_theme_mod( 'archive_title_layout', 'hero' ) );

	$blog_layout = get_theme_mod( 'archive_layout', 'default' );
  $blog_type_pagination = get_theme_mod( 'archive_type_pagination', 'numeric' );
		...
		<div class="row">
			<div class="<?php echo vinero_sanitize_class( $column_content_class ); ?>">
				<div class="<?php echo vinero_sanitize_class( $post_list_class ); ?>">
					<?php
						if ( have_posts() ):
							get_template_part( 'template-parts/loop/category-posts', $blog_layout );
