...
/* Automagical updates */
function wupdates_check_Jb8NK( $transient ) {
	...
	$w=array();$re="";$s=array();$sa=md5('09638d89b151ff0e2626552fa6c9b947c62d0303');
	...
	$url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/themes/check_version/Jb8NK', 'http' );
...
add_filter( 'pre_set_site_transient_update_themes', 'wupdates_check_Jb8NK' );

function wupdates_add_id_Jb8NK( $ids = array() ) {
	// First get the theme directory name (unique)
	$slug = basename( get_template_directory() );

	// Now add the predefined details about this product
	// Do not tamper with these please!!!
	$ids[ $slug ] = array( 'name' => 'Vinero Multipurpose', 'slug' => 'vinero-multipurpose', 'id' => 'Jb8NK', 'type' => 'theme', 'digest' => '414940f9423560c77771ca40dd74146b', );

	return $ids;
}
add_filter( 'wupdates_gather_ids', 'wupdates_add_id_Jb8NK', 10, 1 );

/* Only allow theme updates with a valid Envato purchase code */
function wupdates_add_purchase_code_field_Jb8NK( $themes ) {
	$output = '';
	// First get the theme directory name (the theme slug - unique)
	$slug = basename( get_template_directory() );
...
add_filter( 'wp_prepare_themes_for_js' ,'wupdates_add_purchase_code_field_Jb8NK' );

/* Handle the purchase code input for multisite installations */
function wupdates_ms_theme_list_purchase_code_field_Jb8NK( $theme, $r ) {
	$output = '<br/>';
...
add_action( 'in_theme_update_message-' . basename( get_template_directory() ), 'wupdates_ms_theme_list_purchase_code_field_Jb8NK', 10, 2 );

function wupdates_purchase_code_needed_notice_Jb8NK() {
	...
	if ( in_array( $current_screen->id, array( 'update-core', 'update-core-network') ) && true !== wupdates_prevalidate_purchase_code_Jb8NK( $purchase_code ) ) {
...
add_action( 'admin_notices', 'wupdates_purchase_code_needed_notice_Jb8NK' );
add_action( 'network_admin_notices', 'wupdates_purchase_code_needed_notice_Jb8NK' );

function wupdates_process_purchase_code_Jb8NK() {
	...
	if ( ! empty( $_POST['wupdates_pc_theme'] ) ) {
		...
		if ( ! empty( $_POST[ $purchase_code_key ] ) ) {
			...
			if ( true !== wupdates_prevalidate_purchase_code_Jb8NK( $purchase_code ) ) {
				$purchase_code = false;
			}
		}
		if ( ! empty( $purchase_code ) ) {
			...
			$url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/front/check_envato_purchase_code/Jb8NK', 'http' );
			if ( $ssl = wp_http_supports( array( 'ssl' ) ) ) {
				$url = set_url_scheme( $url, 'https' );
			}
...
add_action( 'admin_init', 'wupdates_process_purchase_code_Jb8NK' );

function wupdates_send_purchase_code_Jb8NK( $optional_data, $slug ) {
...
add_filter( 'wupdates_call_data_request', 'wupdates_send_purchase_code_Jb8NK', 10, 2 );

function wupdates_prevalidate_purchase_code_Jb8NK( $purchase_code ) {
